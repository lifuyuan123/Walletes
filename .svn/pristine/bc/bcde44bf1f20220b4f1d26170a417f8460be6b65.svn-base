...
public final class FragmentUtils {
     ...
     * @param isHide          是否隐藏
     * @param isAddStack      是否入回退栈
     * @param addClass        要加入的fragment.cass
     * @return fragment
     */
    public static Fragment hideAddFragment(@NonNull FragmentManager fragmentManager,
                                           @NonNull Fragment hideFragment,
                                           @NonNull Class<? extends Fragment> addClass,
                                           @IdRes int containerId,
                                           boolean isHide,
                                           boolean isAddStack,
                                           String[] bName, Serializable[] bSerializable,
                                           SharedElement... sharedElement) {
        Fragment addFragment = null;
        try {
            addFragment = addClass.newInstance();
            if (bName != null && bSerializable != null) {
                Bundle bundle = new Bundle();
                for (int i = 0; i < bName.length; i++) {
                    if (bSerializable[i] instanceof Serializable)
                        bundle.putSerializable(bName[i], bSerializable[i]);
                }
