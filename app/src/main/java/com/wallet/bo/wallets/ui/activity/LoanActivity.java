...
public class LoanActivity extends BaseSwipeActivity {
    ...
    protected void setUpView() {
        ...
        {//默认选中第一项
            ...
            String canborrow =getIntent().getStringExtra("canborrow");
            Log.e("canborrow",canborrow);
            //canborrow:  0： 只能借500  / 1： 500-1000
            if(canborrow.equals("0")){
            rgMon.getChildAt(1).setEnabled(false);
            }
        }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.bt_play:
                Log.e("login",MyApplication.getLogin().toString());

                if(tvBankDesc.getText().equals("请添加")){
                    toastShow("请添加收款账户");
                    return;
                }
